...
class HistoryScreenViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        attempts = coreDataHandler.fetchFingerTapGameAttempts(context: context) {
            DispatchQueue.main.async {
                self.gameAttemptsTableView.reloadData()
            }
        }
        highestScoreLabel.text = String(gameHandler.fetchHighstScore())
    }
...
extension HistoryScreenViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = gameAttemptsTableView.dequeueReusableCell(withIdentifier: "GameAttemptCell", for: indexPath) as! GameAttemptCell
        ...
        cell.tapsCompletedLabel.text = String(attempt.tapsCompleted)
        cell.timeTakenLabel.text = String(attempt.timeTaken)
        return cell
    }
